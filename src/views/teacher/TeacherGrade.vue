...
const showDialog = (row) => {
    ...
    scoreModel.value.studentName = row.studentName
    scoreModel.value.courseName = row.courseName
    scoreModel.value.examName = row.examName
    scoreModel.value.score = row.score
...
const updateScore = async() => {
    let result = await updateScoreService(scoreModel.value.id,scoreModel.value.score,scoreModel.value.status)
...
<template>
    <el-card class="page-container">
        ...
        <!-- 对话框 -->
        <el-dialog v-model="dialogVisible" title="录入成绩" width="800">
            <el-form :model="scoreModel" label-width="150px">
                <el-form-item label="考试名称：">{{ scoreModel.examName }}</el-form-item>
                <el-form-item label="学生姓名：">{{ scoreModel.studentName }}</el-form-item>
                <el-form-item label="所属课程：">{{ scoreModel.courseName }}</el-form-item>
                <el-form-item label="录入分数：">
                    <el-input-number v-model="scoreModel.score" placeholder="请录入分数" :precision="2"
                        :step="0.5" style="width: 50%;"></el-input-number>
                </el-form-item>
            </el-form>

            <template #footer>
                <el-button type="info" @click="scoreModel.status='待定'; updateScore()">待定</el-button>
                <el-button type="primary" @click="scoreModel.status='已发布'; updateScore()">发布</el-button>
            </template>
        </el-dialog>
